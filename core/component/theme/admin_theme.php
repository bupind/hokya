...
class Theme extends PoCore
{
				...
				<div class="col-md-4">
					<div class="widget">
						<div class="theme_box" style="background-image:url('../<?=DIR_CON;?>/themes/<?=$theme['folder'];?>/preview.jpg');">
							...
							<ul>
								<?php if (file_exists('../'.DIR_CON.'/assets/'.$theme['folder'].'/config/config.php')) { ?>
		...
		if (!empty($_POST)) {
			if (!empty($_FILES['fupload']['tmp_name'])) {
				...
				if (in_array($exp[1], array('zip'))) {
					...
					if (file_exists('../'.DIR_CON.'/assets/'.$this->postring->valid($_POST['folder'], 'xss'))) {
						...
						if ($archive->extract(PCLZIP_OPT_PATH, '../'.DIR_CON.'/assets/'.$this->postring->valid($_POST['folder'], 'xss')) == 0) {
							unlink('../'.DIR_CON.'/uploads/'.$themeName);
							$this->poflash->error($GLOBALS['_']['theme_message_4'], 'admin.php?mod=theme');
						}
						$data = array(
						...
						$query_theme->execute();
						unlink('../'.DIR_CON.'/uploads/'.$themeName);
						if (file_exists('../'.DIR_CON.'/assets/'.$this->postring->valid($_POST['folder'], 'xss').'/config/install.php')) {
	...
	*/
	public function edit()
	{
		...
		if (!empty($_POST)) {
			if (file_exists('../'.DIR_CON.'/assets/'.$this->postring->valid($_POST['folder'], 'xss').'/'.$this->postring->valid($_POST['file'], 'xss'))){
				$data = $_POST['code_content'];
				$data = str_replace("textareaeska", "textarea", $data);
				$newdata = stripslashes($data);
				if ($newdata != ''){
					$fw = fopen('../'.DIR_CON.'/assets/'.$this->postring->valid($_POST['folder'], 'xss').'/'.$this->postring->valid($_POST['file'], 'xss'), 'w') or die('Could not open file!');
		...
		if (file_exists('../'.DIR_CON.'/assets/'.$this->postring->valid($_GET['folder'], 'xss').'/'.$this->postring->valid($_GET['id'], 'xss'))) {
			$fh = fopen('../'.DIR_CON.'/assets/'.$this->postring->valid($_GET['folder'], 'xss').'/'.$this->postring->valid($_GET['id'], 'xss'), "r") or die("Could not open file!");
			$data = fread($fh, filesize('../'.DIR_CON.'/assets/'.$this->postring->valid($_GET['folder'], 'xss').'/'.$this->postring->valid($_GET['id'], 'xss'))) or die("Could not read file!");
			$data = str_replace("textarea", "textareaeska", $data);
			fclose($fh);
		?>
		<style type="text/css">
					...
					<div class="pull-right" style="margin-top:-70px;">
						<div class="btn-group">
							...
							<?php
								...
								$theme_files = $theme_dir->listDir('../'.DIR_CON.'/assets/'.$this->postring->valid($_GET['folder'], 'xss').'/');
								foreach($theme_files as $theme_file) {
									if ($theme_file != 'index.html' && $theme_file != 'preview.jpg') {
										if (!is_dir('../'.DIR_CON.'/assets/'.$this->postring->valid($_GET['folder'], 'xss').'/'.$theme_file)) {
							?>
	...
	*/
	public function delete()
	{
		...
		if (!empty($_POST)) {
			...
			if (file_exists('../'.DIR_CON.'/assets/'.$theme['folder'].'/config/uninstall.php')) {
				header('location:admin.php?mod=theme&act=uninstall&folder='.$theme['folder']);
			} else {
				$delete_dir = new PoDirectory();
				$delete_folder = $delete_dir->deleteDir("../".DIR_CON."/assets/".$theme['folder']);
				if ($delete_folder) {
	...
	*/
	public function install()
	{
		...
		include_once '../'.DIR_CON.'/assets/'.$_GET['folder'].'/config/install.php';
	}
	...
	*/
	public function uninstall()
	{
		...
		include_once '../'.DIR_CON.'/assets/'.$_GET['folder'].'/config/uninstall.php';
	}
	...
	*/
	public function config()
	{
		...
		include_once '../'.DIR_CON.'/assets/'.$_GET['folder'].'/config/config.php';
	}
