...
$router->match('GET|POST', '/member/login/facebook', function() use ($core, $templates) {
	...
	$loginUrl = $helper->getLoginUrl(BASE_URL.'/'.DIR_CON.'/assets/member/facebook/oauth.php', ['public_profile', 'email', 'manage_pages', 'publish_actions']);
...
$router->match('GET|POST', '/member/login/twitter', function() use ($core, $templates) {
	...
	if ($connection->http_code == 200) {
		...
		header('location:'.BASE_URL.'/'.DIR_CON.'/assets/member/twitter/oauth.php');
	}
});
...
$router->match('GET|POST', '/feed', function() use ($core, $templates) {
	...
	$channel_node->appendChild($xml->createElement("generator", "http://www.eska.org")); //generator
	...
	foreach($recents as $recent) {
		...
		$item_media->setAttribute("url", BASE_URL.'/core/thumbs/'.$recent['picture']);
