...
if (file_exists('install.php')) {
	 ...
	 *
	*/
	include_once 'inc/core/config.php';

	if (VQMOD == TRUE) {
		require_once 'vqmod/vqmod.php';
		VQMod::bootup();
		include_once VQMod::modCheck('inc/core/core.php');
	} else {
	...
	*/
	if (strpos($_SERVER['REQUEST_URI'], DIR_ADM) !== false) {
		...
		*/
		...
		$templates = new PoTemplate\Engine(DIR_CON.'/assets'.$active_template);
		$templates->loadExtension(new PoTemplate\Extension\PoCore());
		$templates->loadExtension(new PoTemplate\Extension\Asset(DIR_CON.'/assets'.$active_template));
